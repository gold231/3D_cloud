...
tree_data = tree_data[tree_data['y'] >= 4182560]
...
build_data = build_data[(build_data['y'] <= 4182559) & (build_data['y'] >= 4182520)]
...
while y <= build_data['x'].max():
    ...
    if len(tree_batch) > 0 and len(build_batch) > 0:
        min_tree_y = tree_batch['y'].min()
        tree_batch = tree_batch[abs(tree_batch['y'] - min_tree_y) < 20 ]
        
        
        max_y = build_batch['y'].max()
        build_batch = build_batch[abs(build_batch['y'] - max_y)<3]
